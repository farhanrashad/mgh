...
class HRExpensePrepayment(models.Model):
    ...
    remaining_amount_residual_b = fields.Monetary('Remainin esidual Amount', store=True, tracking=True)
    ...
    def _compute_remaining_residual_amount(self):
        for exp in self:
            exp.remaining_amount_residual = sum(exp.expense_prepayment_line_ids.mapped('amount_residual'))
            exp.remaining_amount_residual_b = sum(exp.expense_prepayment_line_ids.mapped('amount_residual'))
            ...
            exp.remaining_amount_residual_b = remaining_amount_residual
            if exp.state == 'post':
                exp.remaining_amount_residual = 0
                exp.remaining_amount_residual_b = 0
