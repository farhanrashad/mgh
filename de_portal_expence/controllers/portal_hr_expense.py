...
def expense_page_content(flag = 0, expense=0, categ=0, exception=0, subordinate=0, employee=0, warning=0, forcasted=0, acounting_date=0 ,payment_mode=0):
    ...
    acr_line = request.env['hr.expense.prepayment.line'].sudo().search([('employee_id','=', employees.id),('state','in',('approved','post', 'done'))])
    if employee !=0:
...
#         name = kw.get('name')
    ...
    def create_expenses(self, **kw):
        ...
        acr_line = request.env['hr.expense.prepayment.line'].sudo().search([('employee_id','=', employee.id),('state','in',('approved','post', 'done'))])
